function Show-MainWindow {
    ...
    if ($tabControl -and $tabControl.Items.Count -gt 2) {
        $tabControl.Items[1].Tag = 'AppRemoval'
        $tabControl.Items[2].Tag = 'Tweaks'
    }
    ...
    $appRemovalScopeAllUsers = $window.FindName('AppRemovalScopeAllUsers')
    ...
    # Keep scope behavior independent from the localized ComboBoxItem content.
    if ($appRemovalScopeAllUsers) { $appRemovalScopeAllUsers.Tag = 'AllUsers' }
    if ($appRemovalScopeCurrentUser) { $appRemovalScopeCurrentUser.Tag = 'CurrentUser' }
    if ($appRemovalScopeTargetUser) { $appRemovalScopeTargetUser.Tag = 'TargetUser' }
        ...
        try {
            Import-Configuration -Owner $window -UsesDarkMode $usesDarkMode -AppsPanel $appsPanel -UiControlMappings $script:UiControlMappings -UserSelectionCombo $userSelectionCombo -OtherUsernameTextBox $otherUsernameTextBox -OnAppsImported { Update-AppSelectionStatus -AppsPanel $appsPanel -AppSelectionStatus $appSelectionStatus -AppRemovalScopeCombo $appRemovalScopeCombo -AppRemovalScopeSection $appRemovalScopeSection -AppRemovalScopeDescription $appRemovalScopeDescription -UserSelectionCombo $userSelectionCombo; Update-AppPresetStates -AppsPanel $appsPanel } -OnImportCompleted {
                ...
                $window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Loaded, [action]{
                    Show-Bubble -TargetControl $reviewChangesBtn -Message '在此查看已选更改'
                }) | Out-Null
            }
        }
        catch {
    ...
    $window.Add_KeyDown({
        ...
        if ($e.Key -eq [System.Windows.Input.Key]::F -and
            ...
            if ($currentTab -and $currentTab.Tag -eq 'AppRemoval' -and $appSearchBox) {
                $appSearchBox.Focus()
                $e.Handled = $true
            }
            elseif ($currentTab -and $currentTab.Tag -eq 'Tweaks' -and $tweakSearchBox) {
    ...
    $deploymentApplyBtn.Add_Click({
        ...
        if ($selectedApps.Count -gt 0) {
            ...
            if ($selectedScopeItem) {
                switch ([string]$selectedScopeItem.Tag) {
                    'AllUsers' { AddParameter 'AppRemovalTarget' 'AllUsers' }
                    'CurrentUser' { AddParameter 'AppRemovalTarget' 'CurrentUser' }
                    'TargetUser' { AddParameter 'AppRemovalTarget' ($otherUsernameTextBox.Text.Trim()) }
                }
            }
        }
        ...
        if (-not $hasAppSelection -and $selectedForwardFeatureIds.Count -eq 0 -and $script:UndoParams.Count -eq 0) {
            Show-MessageBox -Message '尚未选择任何更改，请至少选择一项后再继续。' -Title '未选择更改' -Button 'OK' -Icon 'Information'
            return
        }
        ...
        switch ($userSelectionCombo.SelectedIndex) {
            0 { Write-Host "已选用户模式：当前用户 ($(GetUserName))" }
            1 {
                Write-Host "已选用户模式：$($otherUsernameTextBox.Text.Trim())"
                AddParameter User ($otherUsernameTextBox.Text.Trim())
            }
            2 {
                Write-Host "已选用户模式：Windows 默认用户配置文件（Sysprep）"
    ...
    $window.Add_Loaded({
        try {
            ...
            Write-Warning "GUI 初始化时发生错误：$($_.Exception.Message)"
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        $window.Dispatcher.BeginInvoke([System.Windows.Threading.DispatcherPriority]::Loaded, [action]{
            Show-MessageBox -Message "已启用 WhatIf 模式。在此模式下，脚本不会对系统进行任何更改。`n`n您可以在控制台输出中查看脚本将执行的操作。" -Title 'WhatIf 模式' -Button 'OK' -Icon 'Information' -Owner $window
        }) | Out-Null
    }

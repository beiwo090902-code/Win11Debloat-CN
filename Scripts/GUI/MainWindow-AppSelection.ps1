...
function Update-AppSelectionStatus {
    ...
    $AppSelectionStatus.Text = "已选择 $selectedCount 个应用待移除"
...
function Update-AppRemovalScopeDescription {
    ...
    if ($selectedItem) {
        # Content is localized UI text; Tag is the stable scope identifier.
        switch ([string]$selectedItem.Tag) {
            'AllUsers' {
                $AppRemovalScopeDescription.Text = "将为所有用户移除应用，并从 Windows 映像中移除，以防止为新用户重新安装。"
            }
            'CurrentUser' {

...
function Build-DynamicTweaks {
    ...
    if (-not $featuresJson) {
        throw "无法加载 Features.json 文件。缺少功能定义，GUI 无法继续运行。"
    }
    ...
    function GetOrCreateCategoryCard($categoryObj) {
        ...
        $categoryDisplayName = if ($categoryObj.PSObject.Properties['DisplayName']) { $categoryObj.DisplayName } else { ConvertTo-LocalizedText -Text $categoryName }
        ...
        $header.Text = $categoryDisplayName
        ...
        $helpBtn.ToolTip = "打开「${categoryDisplayName}」调整项的 Wiki 说明"
            ...
            if ($item.Type -eq 'group') {
                ...
                if ($filteredValues.Count -eq 1) {
                    ...
                    if ($featureMap.ContainsKey($soleFid)) {
                        ...
                        $items = @('不更改', (ConvertTo-LocalizedText -Text $opt))

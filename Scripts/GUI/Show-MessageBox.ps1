...
function Show-MessageBox {
    ...
    # Display labels may be localized independently; Tag values remain API-compatible.
    $buttonLabels = @{
        OK     = '确定'
        Cancel = '取消'
        Yes    = '是'
    ...
    switch ($Button) {
        'OK' {
            $button1.Content = $buttonLabels.OK
        ...
        'OKCancel' {
            $button1.Content = $buttonLabels.OK
            $button2.Content = $buttonLabels.Cancel
            $button1.Add_Click({ $msgWindow.Tag = 'OK'; $msgWindow.Close() })
            $button2.Add_Click({ $msgWindow.Tag = 'Cancel'; $msgWindow.Close() })
            $button2.Visibility = 'Visible'
        }
        'YesNo' {
            $button1.Content = $buttonLabels.Yes
            $button2.Content = $buttonLabels.No

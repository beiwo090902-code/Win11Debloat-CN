...
function ReplaceStartMenuForAllUsers {
    ...
    Write-Host "> 正在为所有用户移除开始菜单中固定的全部应用..."
...
function ReplaceStartMenu {
    ...
    if (Test-Path $startMenuBinFile) {
        ...
        Write-Verbose "已将用户 $userName 的开始菜单备份保存为 $backupFileName"
    }
    else {
...
function RestoreStartMenuFromBackup {
    ...
    if ($script:Params.ContainsKey("WhatIf")) {
        Write-Host "[WhatIf] 将从备份 $backupBinFile 还原用户 $userName 的开始菜单" -ForegroundColor Cyan
        return [PSCustomObject]@{
            UserName = $userName
            Result = $true

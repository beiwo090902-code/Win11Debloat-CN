...
function DisableStoreSearchSuggestions {
    ...
    {
        Write-Host "找不到用户 $userName 的 Store 应用数据库，正在创建以防止 Windows 稍后创建它..." -ForegroundColor Yellow
...
function EnableStoreSearchSuggestions {
    ...
    try {
        ...
        throw "为用户 $userName 撤销 Microsoft Store 搜索建议时，无法移除 '$StoreAppsDatabase'。$($_.Exception.Message)"
    }
}

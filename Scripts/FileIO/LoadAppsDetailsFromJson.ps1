...
function LoadAppsDetailsFromJson {
    ...
    foreach ($appData in $jsonContent.Apps) {
        ...
        $friendlyName = if ($appData.FriendlyName) { ConvertTo-LocalizedText -Text $appData.FriendlyName } else { $primaryAppId }
        ...
        $apps += [PSCustomObject]@{
            ...
            IsChecked = $isChecked
            Description = ConvertTo-LocalizedText -Text $appData.Description
